...
function parseArgs(argv) {
  const args = {agents: "grok,codex,claude,cursor", root: "."};
...
function upsertTomlBlock(file, block) {
  ...
  writeFileSync(file, `${current}${separator}${START_MARKER}\n${block.trim()}\n${END_MARKER}\n`);
}
...
function persistEnvironment(root) {
  const values = ["HEROUI_KEY", "HEROUI_PERSONAL_TOKEN"]
    .filter((key) => process.env[key])
    .map((key) => `${key}=${process.env[key]}`);
  if (!values.length) throw new Error("--persist-env was requested, but no HeroUI credentials exist in the process environment.");
...
function main() {
  ...
  if (args.help) {
    console.log("Usage: node configure-project.mjs --product <react|native|both> [--root <path>] [--agents grok,codex,claude,cursor] [--persist-env]");
    return;
  }
  ...
  writeFileSync(resolve(agentRoot, ".gitignore"), ".env.local\n");
